...
const SCRAMBLE_OPTION_COUNT = CORNER_COUNT * 2;
let tetrominoTilings = [];

const state = { board: [], initialBoard: [], scramble: [], solution: [], moves: 0, complete: false, cheating: false, clockwise: true, showNumbers: loadNumberPreference() };
let cheatTimer = null;
const elements = {
...
function newPuzzle({ recordUrl = true, cheatAvailable = true } = {}) {
  ...
  do {
    ...
    // Choose both directions. A reversal is disallowed when every intervening
    // move is disjoint, because it would only undo the earlier turn.
    for (let turn = 0; turn < SCRAMBLE_MOVE_COUNT; turn += 1) {
      const move = randomScrambleMove(scramble);
...
function performRotation(row, column, animationDuration = 270, clockwise = true) {
  ...
  state.board = clockwise ? rotateClockwise(state.board, row, column) : rotateAnticlockwise(state.board, row, column);
  state.moves += 1;
  state.complete = isComplete(state.board);
  render();
  animateRotation(row, column, tileRects, animationDuration, clockwise);
}
...
function applyMove(board, move) {
  return move.clockwise ? rotateClockwise(board, move.row, move.column) : rotateAnticlockwise(board, move.row, move.column);
}

function randomScrambleMove(moves) {
  let move;
  do {
    move = { row: Math.floor(Math.random() * (SIZE - 1)), column: Math.floor(Math.random() * (SIZE - 1)), clockwise: Math.random() < .5 };
  } while (undoesUntouchedMove(move, moves));
  return move;
}

function undoesUntouchedMove(move, moves) {
  for (let index = moves.length - 1; index >= 0; index -= 1) {
    const previous = moves[index];
    if (previous.row === move.row && previous.column === move.column) return previous.clockwise !== move.clockwise;
    if (movesOverlap(previous, move)) return false;
  }
  return false;
}

function movesOverlap(first, second) {
    ...
    for (const board of frontier) {
      for (let row = 0; row < SIZE - 1; row += 1) {
        for (let column = 0; column < SIZE - 1; column += 1) {
          next.push(rotateClockwise(board, row, column), rotateAnticlockwise(board, row, column));
        }
      }
    }
    frontier = next;
...
function playSolution() {
  ...
  function nextMove() {
    ...
    state.clockwise = move.clockwise;
    performRotation(move.row, move.column, 350, move.clockwise);
...
function encodeScramble(moves) {
  return moves.reduce((code, move) => code * SCRAMBLE_OPTION_COUNT + (move.row * (SIZE - 1) + move.column) * 2 + Number(!move.clockwise), 0).toString(36);
}

function decodeScramble(token) {
  if (!token || !/^[0-9a-z]+$/i.test(token)) return null;
  let code = Number.parseInt(token, 36);
  if (!Number.isSafeInteger(code) || code >= SCRAMBLE_OPTION_COUNT ** SCRAMBLE_MOVE_COUNT) return null;
  const moves = Array(SCRAMBLE_MOVE_COUNT);
  for (let index = SCRAMBLE_MOVE_COUNT - 1; index >= 0; index -= 1) {
    const option = code % SCRAMBLE_OPTION_COUNT;
    const corner = Math.floor(option / 2);
    moves[index] = { row: Math.floor(corner / (SIZE - 1)), column: corner % (SIZE - 1), clockwise: option % 2 === 0 };
    code = Math.floor(code / SCRAMBLE_OPTION_COUNT);
  }
  return moves;
}
...
function solutionFromScramble(initialBoard, scramble) {
  ...
  const solution = scramble.slice().reverse().map((move) => ({ ...move, clockwise: !move.clockwise }));
  const resultingBoard = solution.reduce(applyMove, initialBoard);
...
function makeRotationButton(row, column) {
  ...
  button.setAttribute("aria-label", `Rotate the four squares around this corner ${state.clockwise ? "clockwise" : "anticlockwise"} (rows ${row + 1}–${row + 2}, columns ${column + 1}–${column + 2})`);
...
function animateRotation(row, column, previousRects, duration = 270, clockwise = true) {
